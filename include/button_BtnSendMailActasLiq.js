Runner.buttonEvents["BtnSendMailActasLiq"] = function( pageObj, proxy, pageid ) {
	//register a new button
	pageObj.buttonNames[ pageObj.buttonNames.length ] = 'BtnSendMailActasLiq';
	
	if ( !pageObj.buttonEventBefore['BtnSendMailActasLiq'] ) {
		pageObj.buttonEventBefore['BtnSendMailActasLiq'] = function( params, ctrl, pageObj, proxy, pageid, rowData, row, submit ) {		
			var ajax = ctrl;
...
return false;
	...
	if ( !pageObj.buttonEventAfter['BtnSendMailActasLiq'] ) {
		pageObj.buttonEventAfter['BtnSendMailActasLiq'] = function( result, ctrl, pageObj, proxy, pageid, rowData, row, params ) {
			var ajax = ctrl;

		}
	}
	
	$('a[id="BtnSendMailActasLiq"]').each( function() {
		if ( $(this).closest('.gridRowAdd').length ) {
			return;
		}
		
		this.id = "BtnSendMailActasLiq" + "_" + Runner.genId();
		
		// create object
		var button_BtnSendMailActasLiq = new Runner.form.Button({
			id: this.id ,
			btnName: "BtnSendMailActasLiq"
		});
		
		// init
		button_BtnSendMailActasLiq.init( {args: [ pageObj, proxy, pageid ]} );
	});
};
